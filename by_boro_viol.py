...
if __name__ == "__main__":
	...
	#aggregate by borough & violation type
	output = df.groupBy(['BORO_NM','LAW_CAT_CD']).count()
	output.rdd.map(lambda x: ",".join(map(str, x))).coalesce(1).saveAsTextFile("crime_boro_type.out")
